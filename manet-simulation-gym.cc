...
bool MyExecuteActions(Ptr < OpenGymDataContainer > action) {
  ...
  uint32_t value = discrete -> GetValue();

  NS_LOG_UNCOND("MyExecuteActions: " << value);
  return true;
}
...
RoutingExperiment::Run(int nSinks, double txp, std::string CSVfileName) {
  ...
  //uint32_t sourceNode = 2; //source node  of cluster 3
  //double envStepTime = 0.6; //seconds, ns3gym env step time interval
  ...
  // TypeId tid = TypeId::LookupByName ("ns3::UdpSocketFactory");
  // Ptr<Socket> source = Socket::CreateSocket (Cluster3.Get (sourceNode), tid);
  // InetSocketAddress remote = InetSocketAddress (interface1.GetAddress (sinkNode, 0), 80);
  // source->Connect (remote);

  // Ptr < Socket > sink = SetupPacketReceive(interfaceList[0].GetAddress(0), listClusters[2].Get(0));

  // AddressValue remoteAddress(InetSocketAddress(interfaceList[2].GetAddress(0), port));
  // onoff1.SetAttribute("Remote", remoteAddress);

  //Ptr < UniformRandomVariable >var = CreateObject < UniformRandomVariable > ();
  //ApplicationContainer temp = onoff1.Install(listClusters[2].Get(3));
  //temp.Start(Seconds(var -> GetValue(100.0, 101.0)));
  //temp.Stop(Seconds(TotalTime));

  Ptr < Socket > sink = SetupPacketReceive(interfaceList[0].GetAddress(0), listClusters[0].Get(0));
  ...
  InetSocketAddress remote = InetSocketAddress (interface3.GetAddress (0, 0), 80);
  ...
  Simulator::Schedule(Seconds(0.0), &ScheduleNextStateRead, envStepTime, openGym,sink,packetSize);
